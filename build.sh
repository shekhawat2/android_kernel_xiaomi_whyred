...
export KERNELZIP=${ANYKERNEL_DIR}/KCUFKernel-whyred-4.4-${BUILD_TIME}.zip
...
if [ x$1 == xgz ]; then
enable_defconfig CONFIG_INPUT_QTI_HAPTICS
build && exit 0
fi
disable_defconfig CONFIG_INPUT_QTI_HAPTICS
build dtbs
enable_defconfig CONFIG_INPUT_QTI_HAPTICS
build dtbs
build Image || exit
...
build Image || exit
${KERNEL_DIR}/bin/bsdiff ${OUT_DIR}/Image ${BUILTIMAGE} ${ANYKERNEL_DIR}/bspatch/newcam.patch || exit
if [ x$1 == xc ]; then
move_files && make_zip
upload_gdrive
fi
